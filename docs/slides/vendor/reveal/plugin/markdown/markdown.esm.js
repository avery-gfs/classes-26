...
const n = /[&<>"']/,
  ...
  o = (e) => l[e];
function a(e, t) {
  if (t) { if (n.test(e)) return e.replace(s, o); }
...
function f(e, t, n) {
  if (e) {
    ...
    if (
      0 === e.indexOf("javascript:") || 0 === e.indexOf("vbscript:") ||
      0 === e.indexOf("data:")
    ) return null;
  }
...
class S {
  ...
  def(e) {
    ...
    if (t) {
      const e = t[1].toLowerCase().replace(/\s+/g, " "),
        n = t[2]
          ? t[2].replace(/^<(.*)>$/, "$1").replace(
            this.rules.inline._escapes,
            "$1",
          )
          : "",
        s = t[3]
          ? t[3].substring(1, t[3].length - 1).replace(
            this.rules.inline._escapes,
            "$1",
          )
          : t[3];
      ...
      return !this.lexer.state.inLink && /^<a /i.test(t[0])
            ...
            (this.lexer.state.inRawBlock = !1),
        {
          ...
          text: this.options.sanitize
            ? this.options.sanitizer ? this.options.sanitizer(t[0]) : a(t[0])
            : t[0],
        };
    }
  }
  link(e) {
    ...
    if (t) {
      ...
      return n = n.trim(),
        /^</.test(n) &&
        (n = this.options.pedantic && !/>$/.test(e)
          ? n.slice(1)
          : n.slice(1, -1)),
        z(
          t,
  ...
  inlineText(e, t) {
    ...
    if (n) {
      let e;
      return e = this.lexer.state.inRawBlock
        ? this.options.sanitize
          ? this.options.sanitizer ? this.options.sanitizer(n[0]) : a(n[0])
          : n[0]
...
R.def = u(R.def).replace("label", R._label).replace("title", R._title)
  .getRegex(),
  R.bullet = /(?:[*+-]|\d{1,9}[.)])/,
  R.listItemStart = u(/^( *)(bull) */).replace("bull", R.bullet).getRegex(),
  R.list = u(R.list).replace(/bull/g, R.bullet).replace(
    "hr",
    "\\n+(?=\\1?(?:(?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$))",
  ...
  R._comment = /<!--(?!-?>)[\s\S]*?(?:-->|$)/,
  R.html = u(R.html, "i").replace("comment", R._comment).replace("tag", R._tag)
    .replace(
      "attribute",
      / +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/,
    ).getRegex(),
  R.paragraph = u(R._paragraph).replace("hr", R.hr).replace(
    "heading",
    " {0,3}#{1,6} ",
  ).replace("|lheading", "").replace("|table", "").replace(
    "blockquote",
    " {0,3}>",
  ...
  R.gfm.table = u(R.gfm.table).replace("hr", R.hr).replace(
    "heading",
    " {0,3}#{1,6} ",
  ...
  R.gfm.paragraph = u(R._paragraph).replace("hr", R.hr).replace(
    "heading",
    " {0,3}#{1,6} ",
  ).replace("|lheading", "").replace("table", R.gfm.table).replace(
    "blockquote",
    " {0,3}>",
  ).replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace(
    "list",
  ...
  R.pedantic = {
    ...
    html: u(
      "^ *(?:comment *(?:\\n|\\s*$)|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)|<tag(?:\"[^\"]*\"|'[^']*'|\\s[^'\"/>\\s]*)*?/?> *(?:\\n{2,}|\\s*$))",
    ).replace("comment", R._comment).replace(
      /tag/g,
    ...
    fences: w,
    lheading: /^(.+?)\n {0,3}(=+|-+) *(?:\n+|$)/,
    paragraph: u(R.normal._paragraph).replace("hr", R.hr).replace(
      "heading",
      " *#{1,6} *[^\n]",
    ).replace("lheading", R.lheading).replace("blockquote", " {0,3}>").replace(
      "|fences",
      "",
...
T._punctuation = "!\"#$%&'()+\\-.,/:;<=>?@\\[\\]`^{|}~",
  T.punctuation = u(T.punctuation).replace(/punctuation/g, T._punctuation)
    .getRegex(),
  T.blockSkip = /\[[^\]]*?\]\([^\)]*?\)|`[^`]*?`|<[^>]*?>/g,
  T.escapedEmSt = /(?:^|[^\\])(?:\\\\)*\\[*_]/g,
  T._comment = u(R._comment).replace("(?:--\x3e|$)", "--\x3e").getRegex(),
  T.emStrong.lDelim = u(T.emStrong.lDelim).replace(/punct/g, T._punctuation)
    .getRegex(),
  T.emStrong.rDelimAst = u(T.emStrong.rDelimAst, "g").replace(
    /punct/g,
    T._punctuation,
  ).getRegex(),
  T.emStrong.rDelimUnd = u(T.emStrong.rDelimUnd, "g").replace(
    /punct/g,
    T._punctuation,
  ).getRegex(),
  T._escapes = /\\([!"#$%&'()*+,\-./:;<=>?@\[\]\\^_`{|}~])/g,
  ...
  T.autolink = u(T.autolink).replace("scheme", T._scheme).replace(
    "email",
    T._email,
  ).getRegex(),
  T._attribute =
    /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/,
  T.tag = u(T.tag).replace("comment", T._comment).replace(
    "attribute",
    T._attribute,
  ).getRegex(),
  T._label = /(?:\[(?:\\.|[^\[\]\\])*\]|\\.|`[^`]*`|[^\[\]\\`])*?/,
  T._href = /<(?:\\.|[^\n<>\\])+>|[^\s\x00-\x1f]*/,
  T._title = /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/,
  T.link = u(T.link).replace("label", T._label).replace("href", T._href)
    .replace("title", T._title).getRegex(),
  T.reflink = u(T.reflink).replace("label", T._label).replace("ref", R._label)
    .getRegex(),
  T.nolink = u(T.nolink).replace("ref", R._label).getRegex(),
  T.reflinkSearch = u(T.reflinkSearch, "g").replace("reflink", T.reflink)
    .replace("nolink", T.nolink).getRegex(),
  T.normal = { ...T },
  T.pedantic = {
    ...T.normal,
    strong: {
    ...
    reflink: u(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", T._label)
      .getRegex(),
  },
  T.gfm = {
    ...T.normal,
  ...
  T.gfm.url = u(T.gfm.url, "i").replace("email", T.gfm._extended_email)
    .getRegex(),
  T.breaks = {
    ...T.gfm,
...
class v {
    ...
    for (
      ...
      if (
        ...
        if (n = this.tokenizer.space(e)) {
          ...
          if (
            r = e, this.options.extensions && this.options.extensions.startBlock
          ) {
            let t = 1 / 0;
            const n = e.slice(1);
            let s;
  ...
  inlineTokens(e, t = []) {
      ...
      if (e.length > 0) {
        for (
          ...
          e.includes(i[0].slice(i[0].lastIndexOf("[") + 1, -1)) &&
            (a = a.slice(0, i.index) + "[" + $("a", i[0].length - 2) + "]" +
    ...
    for (; null != (i = this.tokenizer.rules.inline.escapedEmSt.exec(a));) {
      a = a.slice(0, i.index + i[0].length - 2) + "++" +
    ...
    for (; e;) {
      if (
        ...
        if (n = this.tokenizer.escape(e)) {
          ...
          e = e
            .substring(n.raw.length), t.push(n);
          ...
          this.state.inLink || !(n = this.tokenizer.url(e, E))
        ) {
          if (
            r = e,
...
class I {
  ...
  code(e, t, n) {
    ...
    return e = e.replace(/\n$/, "") + "\n",
      s
        ? '<pre><code class="' + this.options.langPrefix + a(s) + '">' +
          (n ? e : a(e, !0)) + "</code></pre>\n"
                  ...
                  f
                    ? x.tokens.length > 0 && "paragraph" === x.tokens[0].type
                      ? (x.tokens[0].text = w + " " + x.tokens[0].text,
                        x.tokens[0].tokens && x.tokens[0].tokens.length > 0 &&
                        "text" === x.tokens[0].tokens[0].type &&
                        (x.tokens[0].tokens[0].text = w + " " +
  ...
  return (n, s, r) => {
      ...
      l = function (e, t, n) {
        return (s) => {
          if (
            ...
            const e = "<p>An error occurred:</p><pre>" + a(s.message + "", !0) +
              "</pre>";
    ...
    if (s.async) {
      return Promise.resolve(s.hooks ? s.hooks.preprocess(n) : n).then(
        ...
        (e) => s.hooks ? s.hooks.postprocess(e) : e,
      ).catch(l);
    }
    try {
      ...
      if (
        n.async = M.defaults.async || n.async || !1,
          e.extensions && (e.extensions.forEach((e) => {
            ...
            if (e.tokenizer) {
              if (
                !e.level || "block" !== e.level && "inline" !== e.level
              ) {
              ...
              t[e.level]
                ...
                e.start && ("block" === e.level
                  ? t.startBlock
                    ? t.startBlock.push(e.start)
                    : t.startBlock = [e.start]
                  : "inline" === e.level && (t.startInline
      ...
      if (e.hooks) {
        ...
        for (const n in e.hooks) {
          ...
          Z.passThroughHooks.has(n)
            ? t[n] = (r) => {
              if (M.defaults.async) {
                return Promise.resolve(e.hooks[n].call(t, r)).then(
                  (e) => s.call(t, e),
                );
              }
    ...
    for (const s of e) {
      switch (n = n.concat(t.call(M, s)), s.type) {
        ...
        case "list":
          ...
          break;
        default:
          M.defaults.extensions && M.defaults.extensions.childTokens &&
...
const N = "__SCRIPT_END__",
  ...
  B = () => {
    ...
    function t(e) {
      let t =
        (e.querySelector("[data-template]") || e.querySelector("script") || e)
          .textContent;
    ...
    function s(t) {
      ...
      return (t = t || {}).separator = t.separator || n?.separator ||
        "\r?\n---\r?\n",
        t.verticalSeparator = t.verticalSeparator || n?.verticalSeparator ||
          null,
        t.notesSeparator = t.notesSeparator || n?.notesSeparator ||
          "^s*notes?:",
    ...
    function r(e, t) {
      ...
      return 2 === n.length &&
        ...
        '<script type="text/template">' + (e = e.replace(/<\/script>/g, N)) +
        "<\/script>";
    }
    function i(e, t) {
      t = s(t);
      ...
      for (let e = 0, n = p.length; e < n; e++) {
        p[e] instanceof Array
          ? (u += "<section " + t.attributes + ">",
            ...
            u += "</section>")
          : u += "<section " + t.attributes + " data-markdown>" + r(p[e], t) +
            "</section>";
      }
      return u;
    }
              ...
              function (e) {
                return new Promise(function (t, n) {
                  ...
                  try {
                    ...
                    console.warn(
                      "Failed to get the Markdown file " + r +
                ...
                e.outerHTML =
                  ...
                  n + " failed with HTTP status " + t.status +
        ...
        for (let i = 0; i < t.childNodes.length; i++) {
          ...
          "section" === l.nodeName && (o = l, n = l),
            "function" != typeof l.setAttribute &&
              l.nodeType !== Node.COMMENT_NODE || a(o, l, n, s, r);
        }
      }
      t.nodeType === Node.COMMENT_NODE && !1 === o(t, n, s) && o(t, e, r);
    }
    function c() {
      const n = e.getRevealElement().querySelectorAll(
